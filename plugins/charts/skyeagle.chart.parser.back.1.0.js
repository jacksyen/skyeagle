(function($) {
	...
	Chartparser.prototype = {
		...
		_parseNode : function (xmlDoc){
				...
				for(var i = 0; i < chartParam.length; i++){
					...
					if(item.children.length == 0){
						...
						if(reg.test(content)){
							...
							result[itemName] = item.textContent;
						}
						continue;
					}
			...
			var doc = xmlDoc.childNodes;
			var result = [];
			for(var i = 0; i < doc.length; i++){
				var nodes = doc[i].children;
				alert(nodes);
				for(var j = 0; j < nodes.length; j++){
					var chart = nodes[j].children;
					
					if(chart.length == 0){
						continue;
					}
					result.push(parseChart(chart));
				}
			}
			return result;
		},
		...
		init : function() {
			...
			alert($(xmlDoc).find('categories').text());
