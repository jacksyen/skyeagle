...
(function($) {
	var innerToolId = ['collapse','min','max','close'],
		innerToolCls = ['sy-panel-tool-collapse','sy-panel-tool-expand','sy-panel-tool-min','sy-panel-tool-max','sy-panel-tool-close'],
		effects = {anim:true , speed: 'normal'};
	$.widget("ui.panel" , {
		options:{
			// 标题
			title: '',
			// panel自定义的CSS样式
			iconCls : null,
			// panel组件的宽度，可取值为'auto'（默认情况,由浏览器决定宽度），可以取值为'fit'，表示适应父容器的大小（width:100%）。
			width: 'auto',
			/**
		...
		_create: function(){
		    this.element.addClass("sy-panel-body ui-widget-content")
		    	.wrap("<div class='ui-widget sy-panel'></div>");
		},
		_init: function(){
			...
			this._bindEvent();
		 	...
		 	if(options.collapsed !== false){
		 		...
		 		$body.hide();
		 		if(options.header !== false){
		 			$header.find(">.sy-panel-tool >.sy-panel-tool-collapse").removeClass("sy-panel-tool-collapse")
		 				.addClass("sy-panel-tool-expand");
		 		}
		 	}else{
		 		$body.show();
		 		"auto"!==options.height && $parent.height(headerHeight + $body.outerHeight());
		 		if(options.header !== false){
		 			$header.find(">.sy-panel-tool >.sy-panel-tool-expand").removeClass("sy-panel-tool-expand")
		 				.addClass("sy-panel-tool-collapse");
		 		}	
		 	}
		...
		_hide: function($target){
			if("hidden" === this.options._closeMode){
				...
				$target.addClass("ui-helper-hidden-accessible");
			}
		},
		_show: function($target){
			if("hidden" === this.options._closeMode){
				...
				$target.removeClass("ui-helper-hidden-accessible");
			}
		},
		_bindEvent: function(){
			var self = this,
				...
				tools = $body.prev().find("> .sy-panel-tool");
				
			if(options.collapsible){
				tools.find(">.sy-panel-tool-collapse , .sy-panel-tool-expand")
					.click(function(){
						if(options.collapsed !== false){
							self.expand();
						}else{
			...
			if(options.closable !== false){
				tools.find(">.sy-panel-tool-close")
					.click(function(){
						self.close();
					});
			}
		...
		_renderHeader: function(){
			...
			var that = this,
				options = this.options,
				tools = options.tools,
				$header = this.header = $("<div class='sy-panel-header'></div>").insertBefore(this.element);
			if(options.iconCls){
				$("<div class='ui-icon sy-panel-icon'></div>").addClass(options.iconCls).appendTo($header);
			}
			$("<div class='sy-panel-title'></div>").html(options.title).appendTo($header);
			$tool = $("<div class='sy-panel-tool'></div>");
			if(options.collapsible !== false){
				$("<div class='ui-icon sy-panel-tool-collapse'></div>").appendTo($tool);	
			}
			//处理自定义头部右边的工具条
			if($.isArray(tools)){
				for(var i=0,len=tools.length; i<len; i++){
					...
					if(iconCls = this._getInnerToolCls(tool.id)){
						$("<div class='ui-icon'></div>").addClass(iconCls)
							.click(	function(event){
								tool.handler.call(this,that,event);
							}).appendTo($tool);
					}else if(typeof tool.iconCls === 'string'){
						$("<div class='ui-icon'></div>").addClass(tool.iconCls)
							.click(	function(event){
								tool.handler.call(this,that,event);
							}).appendTo($tool);
						...
						(function(tool){
							$("<div class='ui-icon'></div>").addClass(tool.iconCls[0])
								.click(function(event){
									tool.handler.call(this,that,event);
								})
								.hover(function(){
			...
			if(options.closable !== false){
				$("<div class='ui-icon sy-panel-tool-close'></div>").appendTo($tool);	
			}
			//处理内置工具按钮hover时的样式变换
			$tool.find(">div.ui-icon").hover(
				function(){
					var self = this;
		...
		_showLoadingMessage: function(){
			var options = this.options,
				$body = this.element,
				$loadMsg = $body.next(".sy-panel-loadingMessage"),
				position = {
			...
			if($loadMsg.length === 0){
				if("default" === options.loadingMessage){
					$("<div class='sy-panel-loadingMessage'><div class='valignMiddle'><div class='loadingImg'>数据加载中...</div></div></div>")
					.css(position).appendTo($body.parent());
				}else{
					$("<div class='sy-panel-loadingMessage'></div>").appendTo($body.parent())
		...
		_hideLoadingMessage: function(){
			this.element.parent().find(".sy-panel-loadingMessage").hide();
		},
		/**
		 * 设置panel的标题
		...
		setTitle: function(title){
		 	this.element.prev().find(">.sy-panel-title").html(title);
		},
		/**
		 * 设置panel的图标样式
		...
		setIconClass: function(iconCls){
			...
			var $icon = $header.find(">.sy-panel-icon");
		 	if(iconCls == null && $icon.length!==0){
		 		$icon.remove();
		 	}else{
		 		if($icon.length==0){
		 			$icon = $("<div class='ui-icon sy-panel-icon'></div>").insertBefore($header.find(">.sy-panel-title"));
		 		}
		...
		collapse: function(/**anim , speed**/){
			...
			if($header.length !== 0){
				var $expandTool = $header.find("> .sy-panel-tool > div.sy-panel-tool-collapse");
				if($expandTool.length !== 0){
					$expandTool.removeClass("sy-panel-tool-collapse").addClass("sy-panel-tool-expand");
					if($expandTool.hasClass("sy-panel-tool-collapse-hover")){
						$expandTool.toggleClass("sy-panel-tool-collapse-hover sy-panel-tool-expand-hover");
					}
				}
			}
		...
		expand: function(/**anim , speed**/){
			...
			if($header.length !== 0){
				var $expandTool = $header.find("> .sy-panel-tool > div.sy-panel-tool-expand");
				if($expandTool.length !== 0){
					$expandTool.removeClass("sy-panel-tool-expand").addClass("sy-panel-tool-collapse");
					if($expandTool.hasClass("sy-panel-tool-expand-hover")){
						$expandTool.toggleClass("sy-panel-tool-expand-hover sy-panel-tool-collapse-hover");
					}
				}
			}
