(function($) {
	...
	$.widget('ui.slider', {
		...
		_create : function() {
			...
			var slider = ele.find('.slider');
			slider.find('.slider-handle').draggable({
				axis : 'x',
				cursor : "pointer",
				drag : function(e, ui) {
					var movePos = e.pageX; // 391
					
					var eleWidth = ele.offset().left; //208
					
					// if(movePos < 0 || movePos > (slider.width()+eleWidth)){
						// return;
					// }
					if(movePos < eleWidth || movePos > (slider.width()+eleWidth)){
						//return false;
					}
					//var movePos = 
					 var size = opts.min+parseInt(((opts.max-opts.min)/opts.width) *(movePos-eleWidth));
					//var size = opts.min + (opts.max - opts.min) * (movePos / slider.width()).toFixed(0);
					slider.find('.slider-tip').html(size);
				...
				stop : function(event, ui) {
					// var maxWidth = opts.width+ele.offset().left;
					// if(e.pageX > maxWidth){
						// $(this).css({
							// left : maxWidth
						// });
		...
		_bindEvents : function() {
			...
			ele.find('.slider-handle').unbind('.slider').bind('mousedown.slider', function(event) {
				...
				$(this).bind('mousemove.slider',function (e){
					var movePos = e.pageX;
