	...
	$.extend(Datepicker.prototype, {
		...
		_updateDatepicker : function(inst) {
			...
			inst.dpDiv.find('.date-ok').button({
				...
				if(state){
					$.datepicker._selectDay(state.id, state.dateMonth, state.dateYear, state.target);
				}else{
					var dd = new Date();
					var today = inst.dpDiv.find('.ui-datepicker-today');
					$.datepicker._selectDay('#'+inst.id, today.attr('data-month'), today.attr('data-year'), today);
				}
			});
