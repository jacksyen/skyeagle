				...
				if(!opts.multiable){
					...
					parent.find('.sp').bind('click.combo', function() {
						...
						if($(this).hasClass('heightlight')){
							...
							// alert(s.length);
							//val = ['java','c#'];
							//val = 'java,c#,javascript';
							// ele.data('mycombo').val.push(val);
							var temp  = ele.data('mycombo').val;
							if(temp == null || temp === ''){
								ele.data('mycombo').val += val;
							}else{
								ele.data('mycombo').val += ',' + val;
							}
							//ele.data('mycombo').val += val;
						} else {
							//ele.data('mycombo'.val)
							// ele.data('mycombo').val
							// $.removeData('mycombo');
							var temp = ele.data('mycombo').val;
							var t = temp.split(',');
...
// 									
							...
							//alert(t.length);
							// var s = temp.search(temp);
							// alert(s);
						}
						//alert('xxx:'+ele.data('mycombo').val);
						//alert(typeof(ele.data('mycombo').val));
